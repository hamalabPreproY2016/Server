...
# 解析に必要なファイルが存在するかを調査する
def checkExistUseFiles(root_dir) :
    csv_dir = os.path.join(root_dir, 'csv')
    check_list = []
    check_list.append(os.path.join(csv_dir, 'mve.csv'))
    check_list.append(os.path.join(csv_dir, 'emg.csv'))
    check_list.append(os.path.join(csv_dir, 'face.csv'))
    check_list.append(os.path.join(csv_dir, 'voice.csv'))
    check_list.append(os.path.join(csv_dir, 'heartrate.csv'))
    check_list.append(os.path.join(root_dir, 'voice'))
    check_list.append(os.path.join(root_dir, 'image'))
    is_not_found = False
...
def createDataSet(zipfile, lookfile, bodyfile):
    # 展開先ディレクトリを作成
...
# zipを展開
    ...
    # ファイルが存在するか確認する
    if (checkExistUseFiles(target_dir) is False):
        return False
# jsonファイルを作成
