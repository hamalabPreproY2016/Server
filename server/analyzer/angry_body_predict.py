...
MODEL_NAME = os.path.normpath(os.path.join(base, 'angry_body.hdf5')) 
...
if __name__ == '__main__':
    # テストデータ数 x 入力パラメータ数
    X_train = rand(10000, 3)

    y_true  = X_train[:, 0] + X_train[:, 1] * X_train[:, 2] >= 1.2
    y_false = y_true != True
    ...
    y_true  = X_test[:, 0] + X_test[:, 1] * X_test[:, 2] >= 1.2
    y_false = y_true != True
